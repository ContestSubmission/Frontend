<script lang="ts">
    ...
    import { formatDateAndRelative } from "$lib/date_utils";
...
<Popover openFocus>
    <PopoverTrigger asChild let:builder>
        ...
        >
            ...
            {value ? formatDateAndRelative(value) : "Pick a date"}
        </Button>
    </PopoverTrigger>
    <PopoverContent class="flex w-auto flex-col space-y-2 p-2">
        ...
        <div class="flex flex-row items-center">
            ...
            <Input type="number" min="0" max="23" class="size-12 px-1 text-center" bind:value={hours}/>
            <span class="mx-2">:</span>
            <Input type="number" min="0" max="59" class="size-12 px-1 text-center" bind:value={minutes}/>
        </div>
        {#if value === null}
            <FieldErrors>
